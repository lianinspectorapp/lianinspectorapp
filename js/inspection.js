...
function applyOfflineDraftToInspection(draft, { showUi = true, preferChecklist = false } = {}) {
    ...
    deselectedInspectionItemIds.clear();
...
async function restoreOfflineDraft(options = {}) {
    ...
    try {
        ...
        function setItemStatus(itemId, status) {
            const currentStatus = inspectionItemsData[itemId];
            const isDeselecting = currentStatus === status;

            if (isDeselecting) {
                delete inspectionItemsData[itemId];
                deselectedInspectionItemIds.add(String(itemId));
            } else {
                inspectionItemsData[itemId] = status;
                deselectedInspectionItemIds.delete(String(itemId));
            }

            const itemMeta = isDeselecting
                ? (inspectionItemsData[itemId + '_data'] || {})
                : touchInspectionItemData(itemId);
            ...
            if ((isDeselecting || status === 'good') && normalizeDamageSelection(itemMeta.selectedDamage).length > 0) {
        ...
        async function hydrateInspectionStateBeforeSubmit() {
            ...
            deselectedInspectionItemIds.forEach(itemId => {
                delete inspectionItemsData[itemId];
            });
        ...
        function getRequiredInspectionItemsForSubmit() {
            return (sheetItems || []).filter(item => String(item?.id || '').trim() !== '');
        }

        function getCompletedInspectionItemCount() {
            return getRequiredInspectionItemsForSubmit().filter(item => {
                const value = inspectionItemsData?.[item.id];
                return value === 'good' || value === 'warning' || value === 'bad';
            }).length;
        }
        ...
        async function submitInspection(e) {
            ...
            const total = getRequiredInspectionItemsForSubmit().length;
            ...
            console.log('[7] Proceeding with submission');
            const scoreResult = getCurrentInspectionScoreResult();
...
// =============================================================
// V127 - Report inspection scope disclaimer
// =============================================================
// Disclaimer ini ditempatkan tepat di bawah bar nama client pada laporan.
(function installLianV127ReportDisclaimer() {
    if (window.__LIAN_V127_REPORT_DISCLAIMER_INSTALLED) return;
    window.__LIAN_V127_REPORT_DISCLAIMER_INSTALLED = true;

    const DISCLAIMER_ID = 'lianReportInspectionDisclaimerV127';
    const DISCLAIMER_TEXT = 'Laporan hasil inspeksi ini diperoleh berdasarkan pemeriksaan kendaraan dari item-item penilaian dibawah dan tidak mencakup bagian diluar item tersebut';

    function escapeV127(value) {
    ...
    function buildDisclaimerHtmlV127() {
        return `
            <div id="${DISCLAIMER_ID}" style="margin:-6px 0 16px; background:#fff7ed; border:1.5px solid #fed7aa; border-left:6px solid #f59e0b; border-radius:16px; padding:11px 13px; color:#7c2d12; font-size:11.5px; font-weight:850; line-height:1.5; break-inside:avoid; page-break-inside:avoid;">
                <div style="display:flex; gap:9px; align-items:flex-start;">
                    <div style="width:24px; height:24px; border-radius:999px; background:#ffedd5; color:#c2410c; display:flex; align-items:center; justify-content:center; font-weight:1000; flex:0 0 auto;">!</div>
                    <div style="min-width:0;">
                        <div style="font-size:11px; font-weight:1000; color:#9a3412; text-transform:uppercase; letter-spacing:.04em; margin-bottom:2px;">Perhatian</div>
                        <div>${escapeV127(DISCLAIMER_TEXT)}</div>
                    </div>
                </div>
            </div>
        `;
    ...
    function injectDisclaimerV127() {
        ...
        reportContent.querySelector(`#${DISCLAIMER_ID}`)?.remove();
        clientBar.insertAdjacentHTML('afterend', buildDisclaimerHtmlV127());
    }

    function scheduleDisclaimerV127() {
        [120, 420, 900, 1700, 3000].forEach(delay => {
            setTimeout(injectDisclaimerV127, delay);
        });
    }
    ...
    try {
        ...
        if (previousGenerateAndShowReport) {
            generateAndShowReport = function generateAndShowReportV127() {
                const result = previousGenerateAndShowReport.apply(this, arguments);
                scheduleDisclaimerV127();
                return result;
            };
        }
    } catch (err) {
        console.warn('[v127-disclaimer] patch generateAndShowReport dilewati:', err?.message || err);
    }

    window.LianReportDisclaimerV127 = {
        inject: injectDisclaimerV127
    };

    console.log('âœ… inspection.js v127 report disclaimer loaded');
})();


// =============================================================
// V126 - Editable persistent category scores on report
// =============================================================
// Tujuan:
// - Score "Kondisi per Kategori" di laporan dapat diedit manual.
// - Nilai edit disimpan ke inspections.category_scores agar report berikutnya
//   memakai nilai yang sudah diset user, bukan dihitung ulang.
(function installLianV126EditableCategoryScores() {
    if (window.__LIAN_V126_EDITABLE_CATEGORY_SCORES_INSTALLED) return;
    window.__LIAN_V126_EDITABLE_CATEGORY_SCORES_INSTALLED = true;

    const TAG = '[v126-category-scores]';
    const STORAGE_PREFIX = 'lian_category_scores_';

    function cleanTextV126(value) {
        return String(value ?? '').replace(/\s+/g, ' ').trim();
    }

    function normalizeKeyV126(value) {
        return cleanTextV126(value).toLowerCase();
    }

    function clampScoreV126(value) {
        const digits = String(value ?? '').replace(/[^0-9]/g, '').slice(0, 3);
        const number = Number(digits);
        if (!Number.isFinite(number)) return 0;
        return Math.max(0, Math.min(100, Math.round(number)));
    }

    function colorForScoreV126(score) {
        const n = clampScoreV126(score);
        if (n >= 80) return '#16a34a';
        if (n >= 50) return '#d97706';
        return '#dc2626';
    }

    function getInspectionIdV126(inspection = {}) {
        return String(
    ...
    function parseCategoryScoresPayloadV126(value) {
        if (!value) return {};
        ...
        if (parsed.scores && typeof parsed.scores === 'object') return parsed.scores;
        return parsed;
    }

    function getStoredCategoryScoresV126(inspection = {}) {
        const inspectionId = getInspectionIdV126(inspection);
        let scores = parseCategoryScoresPayloadV126(
            inspection?.category_scores ??
            inspection?.categoryScores ??
            inspection?._categoryScores
        );

        if (Object.keys(scores).length === 0 && inspectionId) {
            try {
                scores = parseCategoryScoresPayloadV126(localStorage.getItem(STORAGE_PREFIX + inspectionId));
            } catch (_) {}
        }
    ...
    function getSavedScoreForCategoryV126(scores = {}, categoryName = '') {
        const exact = scores[categoryName];
        if (exact !== undefined && exact !== null && exact !== '') {
            return clampScoreV126(typeof exact === 'object' ? exact.score : exact);
        }

        const targetKey = normalizeKeyV126(categoryName);
        const matchKey = Object.keys(scores || {}).find(key => normalizeKeyV126(key) === targetKey);
        if (!matchKey) return null;

        const value = scores[matchKey];
        if (value === undefined || value === null || value === '') return null;
        return clampScoreV126(typeof value === 'object' ? value.score : value);
    }

    function getCategoryNameFromCardV126(card) {
        const title = card?.querySelector('[data-lian-category-score-title="true"]') ||
            card?.querySelector('div[style*="font-size:12px"]');
        return cleanTextV126(title?.textContent || '');
    }

    function getScoreNodeFromCardV126(card) {
        return card?.querySelector('[data-lian-category-score-value="true"]') ||
            Array.from(card?.querySelectorAll('div') || []).find(el => /%$/.test(cleanTextV126(el.textContent || '')) && cleanTextV126(el.textContent || '').length <= 4);
    }

    function getBarNodeFromCardV126(card) {
        return card?.querySelector('[data-lian-category-score-bar="true"]') ||
            card?.querySelector('div[style*="height:100%"][style*="width"]');
    }

    function refreshCardScoreV126(card, score) {
        const cleanScore = clampScoreV126(score);
        const color = colorForScoreV126(cleanScore);
        const scoreNode = getScoreNodeFromCardV126(card);
        const barNode = getBarNodeFromCardV126(card);

        if (scoreNode) {
            scoreNode.textContent = String(cleanScore);
            scoreNode.style.color = color;
            scoreNode.style.borderBottomColor = `${color}55`;
        }

        const percent = scoreNode?.nextElementSibling;
        if (percent && cleanTextV126(percent.textContent) === '%') {
            percent.style.color = color;
        }

        if (barNode) {
            barNode.style.width = `${cleanScore}%`;
            barNode.style.maxWidth = '100%';
            barNode.style.background = color;
        }

        card.dataset.lianCategoryScore = String(cleanScore);
    }

    function makeCardEditableV126(card, inspection = {}) {
        if (!card || card.dataset.lianCategoryScoreBound === 'true') return;

        const categoryName = getCategoryNameFromCardV126(card);
        let scoreNode = getScoreNodeFromCardV126(card);
        const barNode = getBarNodeFromCardV126(card);
        if (!categoryName || !scoreNode) return;

        if (!scoreNode.dataset.lianCategoryScoreValue && /%/.test(cleanTextV126(scoreNode.textContent || ''))) {
            const score = clampScoreV126(scoreNode.textContent);
            const color = colorForScoreV126(score);
            scoreNode.innerHTML = `<span data-lian-category-score-value="true" contenteditable="true" spellcheck="false" inputmode="numeric" title="Klik untuk edit score kategori" style="outline:none;cursor:text;color:${color};border-bottom:2px dashed ${color}55;display:inline-block;min-width:34px;">${score}</span><span style="color:${color};">%</span>`;
            scoreNode = scoreNode.querySelector('[data-lian-category-score-value="true"]');
        }

        card.dataset.lianCategoryName = categoryName;
        card.dataset.lianInspectionId = getInspectionIdV126(inspection);
        card.dataset.lianCategoryScoreBound = 'true';

        scoreNode.dataset.lianCategoryScoreValue = 'true';
        scoreNode.setAttribute('contenteditable', 'true');
        scoreNode.setAttribute('spellcheck', 'false');
        scoreNode.setAttribute('inputmode', 'numeric');
        scoreNode.setAttribute('title', 'Klik untuk edit score kategori');
        scoreNode.style.outline = 'none';
        scoreNode.style.cursor = 'text';
        scoreNode.style.borderBottom = `2px dashed ${colorForScoreV126(scoreNode.textContent)}55`;
        scoreNode.style.display = 'inline-block';
        scoreNode.style.minWidth = '34px';
        scoreNode.textContent = String(clampScoreV126(scoreNode.textContent));

        if (barNode) barNode.dataset.lianCategoryScoreBar = 'true';

        const status = document.createElement('div');
        status.dataset.lianCategoryScoreStatus = 'true';
        status.style.cssText = 'font-size:10px;font-weight:850;color:#64748b;margin-top:7px;min-height:12px;';
        status.textContent = 'Klik angka untuk edit';
        card.appendChild(status);

        const commit = () => {
            refreshCardScoreV126(card, scoreNode.textContent);
            saveCurrentCategoryScoresV126(inspection);
        };

        scoreNode.addEventListener('keydown', event => {
            if (event.key === 'Enter') {
                event.preventDefault();
                scoreNode.blur();
            }
        ...
        scoreNode.addEventListener('input', () => {
            const raw = String(scoreNode.textContent || '').replace(/[^0-9]/g, '').slice(0, 3);
            if (String(scoreNode.textContent || '') !== raw) scoreNode.textContent = raw;
        });

        scoreNode.addEventListener('blur', commit);
        refreshCardScoreV126(card, scoreNode.textContent);
    }

    function collectCurrentCategoryScoresV126() {
        const scores = {};
        document.querySelectorAll('#reportContent .report-category-score').forEach(card => {
            const categoryName = card.dataset.lianCategoryName || getCategoryNameFromCardV126(card);
            const scoreNode = getScoreNodeFromCardV126(card);
            if (!categoryName || !scoreNode) return;
            scores[categoryName] = clampScoreV126(scoreNode.textContent);
        });
        return scores;
    }

    function updateCategoryScoresInMemoryV126(inspectionId, payload) {
        ...
        const patch = item => {
            ...
            if (String(itemId || '') === id) {
                item.category_scores = payload;
                item.categoryScores = payload;
                item._categoryScores = payload;
            }
        };
        ...
        const active = window.__lianCurrentReportInspectionV126;
        if (active && getInspectionIdV126(active) === id) {
            active.category_scores = payload;
            active.categoryScores = payload;
            active._categoryScores = payload;
        }
    }

    async function saveCategoryScoresToSupabaseV126(inspectionId, payload) {
        const id = String(inspectionId || '').trim();
        if (!id) return false;
        ...
        updateCategoryScoresInMemoryV126(id, payload);
        ...
        const { error } = await supabaseClient
            .from('inspections')
            .update({ category_scores: payload })
            .eq('id', id);
    ...
    async function saveCurrentCategoryScoresV126(inspection = {}) {
        const id = getInspectionIdV126(inspection) || getInspectionIdV126(window.__lianCurrentReportInspectionV126 || {});
        if (!id) return;

        const payload = {
            version: 'v126',
            scores: collectCurrentCategoryScoresV126(),
            updatedAt: new Date().toISOString(),
            updatedBy: currentUser?.username || currentUser?.id || ''
        };

        document.querySelectorAll('#reportContent [data-lian-category-score-status="true"]').forEach(el => {
            el.textContent = 'Menyimpan score kategori...';
            el.style.color = '#64748b';
        });

        try {
            const synced = await saveCategoryScoresToSupabaseV126(id, payload);
            document.querySelectorAll('#reportContent [data-lian-category-score-status="true"]').forEach(el => {
                el.textContent = synced ? 'Score kategori tersimpan' : 'Score kategori tersimpan lokal';
                el.style.color = synced ? '#16a34a' : '#d97706';
            });
        } catch (err) {
            console.warn(TAG, 'gagal simpan category_scores:', err?.message || err);
            document.querySelectorAll('#reportContent [data-lian-category-score-status="true"]').forEach(el => {
                el.textContent = 'Tersimpan lokal, gagal sync';
                el.style.color = '#dc2626';
            });
        }
    }

    function applyStoredScoresV126(inspection = {}) {
        const storedScores = getStoredCategoryScoresV126(inspection);

        document.querySelectorAll('#reportContent .report-category-score').forEach(card => {
            const categoryName = getCategoryNameFromCardV126(card);
            const savedScore = getSavedScoreForCategoryV126(storedScores, categoryName);
            makeCardEditableV126(card, inspection);
            if (savedScore !== null) refreshCardScoreV126(card, savedScore);
        });
    }

    function scheduleEnhanceCategoryScoresV126(inspection = {}) {
        [60, 260, 720, 1400, 2600].forEach(delay => {
            setTimeout(() => applyStoredScoresV126(inspection || window.__lianCurrentReportInspectionV126 || {}), delay);
        });
    }
    ...
    try {
        ...
        if (previousGenerateAndShowReport) {
            generateAndShowReport = function generateAndShowReportV126(inspection) {
                window.__lianCurrentReportInspectionV126 = inspection || {};
                const result = previousGenerateAndShowReport.apply(this, arguments);
                scheduleEnhanceCategoryScoresV126(inspection || {});
                return result;
            };
        }
    } catch (err) {
    ...
    window.LianEditableCategoryScoresV126 = {
        enhance: applyStoredScoresV126,
        save: saveCurrentCategoryScoresV126,
        collect: collectCurrentCategoryScoresV126,
        parse: parseCategoryScoresPayloadV126
    };

    console.log('âœ… inspection.js v126 editable persistent category scores loaded');
})();
